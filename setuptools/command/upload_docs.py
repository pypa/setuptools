...
class upload_docs(upload):
    ...
    user_options = [
        (
            'repository=',
            'r',
            "url of repository [default: {}]".format(upload.DEFAULT_REPOSITORY),
        ),
    ...
    def finalize_options(self):
        ...
        self.announce('Using upload directory {}'.format(self.target_dir))
    ...
    def run(self):
        ...
        zip_file = os.path.join(tmp_dir, "{}.zip".format(name))
        try:
    ...
    def upload_file(self, filename):
        ...
        msg = "Submitting documentation to {}".format(self.repository)
        ...
        if r.status == 200:
            ...
            if location is None:
                location = 'https://pythonhosted.org/{}/'.format(meta.get_name())
            msg = 'Upload successful. Visit {}'.format(location)
            self.announce(msg, log.INFO)
        else:
