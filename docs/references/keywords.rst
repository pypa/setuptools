...
``include_package_data``
    If set to ``True``, package data file paths are populated from
    ``MANIFEST.in`` for wheel distributions, in addition to ``package_data``.

    **If set to ``True``, the value of ``package_data`` is ignored
    for source distributions.**

    For more information, see the section on :ref:`Including Data Files`.

``exclude_package_data``
    A mapping of package names to lists of glob patterns of data files
    inside package directories that should be excluded from the distribution.
    You can use this to trim back any excess files from
    ``include_package_data``.

    **This option has lower precedence than ``MANIFEST.in`` for source
    distributions.**
...
``package_data``
    A mapping of package names to lists of glob patterns of data files
    inside package directories that should be included in the distribution.

    **This option is ignored for source distributions
